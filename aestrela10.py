    ...
    for j in range(4):
        for k in range(4):
            for l in range(k+1,4):
                if (k == l):
                    continue
                if ((line[read_config[4*j + k]] == line[read_config[4*j + l]]) and ((final_position[read_config[4*j + k]] > final_position[read_config[4*j + l]]))):
                    resp += 2
                if ((row[read_config[j + 4*k]] == row[read_config[j + 4*l]]) and ((final_position[read_config[j + 4*k]] > final_position[read_config[j + 4*l]] and j < k))):
                    resp += 2
    return resp
