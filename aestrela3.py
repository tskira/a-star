...
def HeuristicTree(read_config):
    return sum(map(lambda i: memo_heuristic[i[1]][i[0]],
                   filter(lambda x: x[1] != 0, enumerate(read_config))))
    
def GetChildren(current_config):
    zero_position = current_config[3]
    for i in swap_position[zero_position]:
            aux_current = list(current_config[2])
            aux_current[zero_position], aux_current[i] = aux_current[i], aux_current[zero_position]
            aux_current = tuple(aux_current)
            if aux_current not in closed_list:
                heapq.heappush(open_list, (HeuristicTree(aux_current) + current_config[1] + 1,
                                           current_config[1] + 1, aux_current, i))

def AStar(initial_config):
    heapq.heappush(open_list,(HeuristicTree(initial_config), 0, initial_config, initial_config.index(0)))
    current_state = heapq.heappop(open_list)
    while(current_state[0] - current_state[1]):
        closed_list.add(current_state[2])
        GetChildren(current_state)
        current_state = heapq.heappop(open_list)
