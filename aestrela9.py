...
def HeuristicTree(read_config):
    ...
    for i in range(16):
        if (read_config[i] != 0):
            ...
            if (final_position[read_config[i]] != i):
                zero_neighbor = False
                index = 0
                while(not zero_neighbor and index < len(swap_position[i])):
                    zero_neighbor = read_config[swap_position[i][index]] == 0
                    index += 1
                if(not zero_neighbor):
                    if((line[read_config[i]] == i // 4)):
                        resp += 2
                zero_neighbor = False
                index = 0
                while(not zero_neighbor and index < len(swap_position[i])):
